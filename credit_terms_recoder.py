...
df.drop([
    ...
    "Requirements: Other", "Purchase APR Offered?", "Purchase APR Vary by Credit Tier"
], axis=1, inplace=True)
...
for fixes in [("APR", ""), ("Intro APR", ".1")]:
    for apr_mask in [("1", "1", "Maximum"), ("2", "1|2", "Median"), ("3", "1|2|3", "Minimum")]:
        name = fixes[0] + " Credit " + apr_mask[0]
        df.loc[df[name].isnull() & df['Targeted Credit Tiers'].str.contains(apr_mask[1]), name] = df[apr_mask[2] + " APR" + fixes[1]]
...
df.drop(["Introductory APR Vary By Credit Tier"], axis=1, inplace=True)
df.rename(columns = {"Median Length of Introductory APR": "Intro APR Length"}, inplace=True)
